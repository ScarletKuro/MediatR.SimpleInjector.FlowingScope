...
using MediatR.SimpleInjector.FlowingScope.Internal;
using SimpleInjector;

namespace MediatR.SimpleInjector.FlowingScope;

/// <summary>
/// Behavior for executing all <see cref="IRequestExceptionHandler{TRequest,TResponse}"/>
...
public class RequestExceptionProcessorBehaviorScoped<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
    where TRequest : IRequest<TResponse>
{
    private readonly ServiceFactory _serviceFactory;

    public RequestExceptionProcessorBehaviorScoped(Scope scope)
    {
        _serviceFactory = scope.GetInstance;
    }
        ...
        catch (Exception exception)
        {
            ...
            while (!state.Handled && exceptionType != typeof(Exception))
            {
                ...
                foreach (var exceptionHandler in exceptionHandlers)
                {
                    try
                    {
                        await ((Task)(handleMethod.Invoke(exceptionHandler,
                                          new object[] { request, exception, state, cancellationToken })
                                      ?? throw new InvalidOperationException(
                                          "Did not return a Task from the exception handler.")))
                            .ConfigureAwait(false);
                    }
                    catch (TargetInvocationException invocationException) when
    ...
    private IList<object> GetExceptionHandlers(TRequest request, Type exceptionType,
        out MethodInfo handleMethodInfo)
    {
        var exceptionHandlerInterfaceType =
            typeof(IRequestExceptionHandler<,,>).MakeGenericType(typeof(TRequest), typeof(TResponse),
                exceptionType);
        ...
        var exceptionHandlers =
            (IEnumerable<object>)_serviceFactory.Invoke(enumerableExceptionHandlerInterfaceType);
